...
void APP_Tasks (void )
{
    ...
    signed char accelerometer_data[2], buffer[100], buffer1[100], buffer2[100];
    int i, jj = 2;
            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    ...
                    case 1:
                        ...
                        BSP_LEDToggle( 15 );
                        char message[24];
                        int message_row_number, kk = 1;
                        for (i=0; i <= 24; i++){
                            message[i] = appData.receiveDataBuffer[kk];
                            kk = kk + 1;
                        }
                        message_row_number = appData.receiveDataBuffer[26];
                        display_write(0, message_row_number, message);
                        appData.hidDataReceived = false;
                    ...
                    case 2:
                        ...
                        {
                            appData.transmitDataBuffer[0] = 2;
                            ...
                            if(_CP0_GET_COUNT()<200000){
                                ...
                                for (i=0; i<=2; i++){
                                    accelerometer_data[0] = accels[i] & 0xff;
                                    appData.transmitDataBuffer[jj] = accelerometer_data[0];
                                    jj = jj+1;
                                    accelerometer_data[1] = (accels[i]>>8) & 0xff;
                                    appData.transmitDataBuffer[jj] = accelerometer_data[1];
                                    jj = jj+1;
                                }
                                _CP0_SET_COUNT(0);
                            }
