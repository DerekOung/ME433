    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                sscanf(rxbuffer, "%d", &COMcoordinate);
                
               
                if (COMcoordinate >= 250 & COMcoordinate <= 390){
                    // On line, Accelerate
                ...
                else if (COMcoordinate >= 220 & COMcoordinate < 250){
                    // Turn Soft Left
                    LATBbits.LATB7 = 1; // LEFT
                    LATBbits.LATB14 = 1; // RIGHT
                    OC1RS = (PR2+1)*0.5;
                    OC2RS = abs(COMcoordinate - 640)*(PR2+1)/640.0 * 0.5;
                }
                else if (COMcoordinate <= 420 & COMcoordinate > 390){
                    // Turn Soft Right
                    LATBbits.LATB7 = 1; // LEFT
                    LATBbits.LATB14 = 1; // RIGHT
                    OC1RS = abs(COMcoordinate - 640)*(PR2+1)/640.0 * 0.5;
                    OC2RS = (PR2+1)*0.5;
                }
                
                else if (COMcoordinate < 220){
                    // Turn Hard Left
                    LATBbits.LATB7 = 0; // LEFT
                    LATBbits.LATB14 = 1; // RIGHT
                    OC1RS = (PR2+1)*0.25;
                    OC2RS = (PR2+1)*0.25;
                }
                else if (COMcoordinate > 420){
                    // Turn Hard Right
                    LATBbits.LATB7 = 1; // LEFT
                    LATBbits.LATB14 = 0; // RIGHT
                    OC1RS = (PR2+1)*0.25;
                    OC2RS = (PR2+1)*0.25;
                }
            ...
            /* Remote Control
                ...
                else if (appData.readBuffer[0] == 'a') {
                    // turn left
                    OC1RS = (PR2+1);
                    OC2RS = 0.5*(PR2+1);
                }
                else if (appData.readBuffer[0] == 's') {
                    // brake
                    OC1RS = 0;
                    OC2RS = 0;
                }
        ...
        case APP_STATE_CHECK_UART_RECEIVE:
            ...
            if(_CP0_GET_COUNT() >= 20000000){    //Delay be 20 M core ticks, 1s
                writeRequestResult = USB_DEVICE_CDC_Write(0, &appData.writeTransferHandle, message, 64, USB_DEVICE_CDC_TRANSFER_FLAGS_DATA_COMPLETE);
                            // message[1] = counter++;
                if(USB_DEVICE_CDC_RESULT_OK != writeRequestResult) {  
                    // LATBbits.LATB7 = 1; // turn on another led if there is an error  
                }  
                else {  
